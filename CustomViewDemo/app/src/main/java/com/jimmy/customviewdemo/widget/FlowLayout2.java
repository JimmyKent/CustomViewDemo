...
public class FlowLayout2 extends ViewGroup {
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < lineNum; i++) {
			...
			for (int j = 0; j < lineViews.size(); j++) {//放置每行里面的view 要改变子view的padding
				View child = lineViews.get(j);
				if (child.getVisibility() == View.GONE) {
					continue;
				}
				if (lineWidth < mLayoutWidth - getPaddingLeft() - getPaddingRight()) {
					int addPadding = (mLayoutWidth - getPaddingLeft() - getPaddingRight() - lineWidth) //减去padding
							/ lineViews.size()//
							/ 2;//左右所以除以2
